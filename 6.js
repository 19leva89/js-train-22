// Ланцюжок відповідальності (Chain of Responsibility) — це паттерн програмування, який дозволяє передавати запити послідовно через ланцюжок обробників, 
...
class AuthProcessor {
	...
	// Якщо наступного обробника немає, повертає false, сигналізуючи про невдалу автентифікацію.
...
class TwoStepProcessor extends AuthProcessor {
	...
	// Виводить повідомлення про успішну автентифікацію: Вхід дозволено з двофакторною автентифікацією, і повертає true.
	// Якщо дані не вірні, запит на автентифікацію передається наступному обробнику в ланцюгу, super.validate(username, passkey).
	validate(username, passkey) {
